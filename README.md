...
**NB!** All files in the modelfit are expected to be motion corrected and registered to the same reference space!! A better way to do this is to register motion corrected files to a standard space first using the `bids_warp2nmt_workflow.py` workflow. 

   * The `--csv` flag is mandatory and should point to the csv-file that is formatted as explained above.  
   * Here you can again specify a *refsubject* column in the csv. Make sure this is done correctly as it determines which brainmask file will be used in the analysis.
