...
def create_images_workflow(do_hires):
    iso_voxsize = (1.0, 1.0, 1.0)
    hires_voxsize = (0.6, 0.6, 0.6)
    
    # the meain workflow
    workflow = Workflow(name='minimal_proc')
    inputs = Node(IdentityInterface(fields=['images']), name="in")
    outputs_minproc = Node(IdentityInterface(fields=['images']), name="out")
    outputs_iso = Node(IdentityInterface(fields=['images']), name="out")
    ...
    if do_hires:
        hires = MapNode(fs.MRIConvert(vox_size=hires_voxsize),
            iterfield=['in_file'], name='hires')
        workflow.connect(ro, 'out_file', iso, 'in_file')
        workflow.connect(hires, 'out_file', outputs_iso, 'images')
...
if in_file['ev']:
    ...
    evfiles = Node(nio.SelectFiles({
                ...
                'sub-{subject_id}_ses-{session_id}_*run-{run_id}_*events/Log_*_eventlog.csv',
                'stim_dir':
                'sourcedata/sub-{subject_id}/ses-{session_id}/func/'
                'sub-{subject_id}_ses-{session_id}_*run-{run_id}_*events/',
...
if in_file['func'] or in_file['other']:
    do_hires = False
    minproc_def = create_images_workflow(do_hires)
    workflow.connect(ds_func, 'images', minproc_def, 'in.images')
    workflow.connect(ds_other, 'images', minproc_def, 'in.images')
    workflow.connect(minproc_def, 'out.images',
...
if in_files['ev']:
    ...
    workflow.connect(ds_ev, 'csv_eventlogs', csv2tsv, 'in_file')
...
if in_file['anat']:
    workflow.connect(ds_anat,'anat_files',)
if in_file['func']:
if in_file['ev']:
